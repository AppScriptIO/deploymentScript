...

{
  ...
  /** docker-compose */
  ;`docker-compose -f ${ymlFile} up -d --force-recreate ${serviceName}`
  ;`docker-compose -f ${ymlFile} build --no-cache ${serviceName}`
  ;[`docker-compose`, `-f ${ymlFile}`, `--project-name ${containerPrefix}`, `run --service-ports --use-aliases`, `--entrypoint '${containerCommand}'`, `${serviceName}`]
  ;[
    'docker-compose',
    `-f ${ymlFile}`,
  ...
  ;[`docker-compose -f ${dockerComposeFilePath}`, `--project-name ${projectName}`, `up`, `--no-build`, `--force-recreate`, `--abort-on-container-exit`, `${serviceName}`]
}
