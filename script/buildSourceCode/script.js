...
exports.build = adapter;
exports.createGithubBranchedRelease = createGithubBranchedRelease;
...
async function createGithubBranchedRelease({
  ...
  // branch used to build source code and create a distribution tag from
  brachToPointTo = 'master',
  // default branch for latest commit.
  commitToPointTo = null,
  // unrelated commit to point to
  tagName,
  buildCallback = _buildTool.build // build async function that will handle building source code and preparing the package for distribution.
  ...
  brachToPointTo = await _nodegit.default.Branch.lookup(repository, brachToPointTo, 1); // convert to branch reference
  // set commit reference

  commitToPointTo = Boolean(commitToPointTo) ? await _nodegit.default.Commit.lookup(repository, commitToPointTo) // get commit from supplied commit id parameter
  : await repository.getReferenceCommit(brachToPointTo); // get latest commit from branch
  // get all branches remote and local

  let branchReferenceList = await repository.getReferences(_nodegit.default.Reference.TYPE.OID); // check if `temporaryBranchName` branch, that is used, exists.
  ...
  await _nodegit.default.Reset.reset(repository, commitToPointTo, _nodegit.default.Reset.TYPE.HARD).then(number => {
    if (number) throw new Error(`• Could not reset repository ${repository} to commit ${commitToPointTo}`);
  }).catch(error => console.error); // run build

  await buildCallback({
    targetProjectRoot
  }).then(() => console.log('Project built successfully !')); // Create commit of all files.
  ...
  await repository.createCommit('HEAD'
  /* update the HEAD reference - so that the HEAD will point to the latest git */
  || null
  /* do not update ref */
  , tagger, tagger, `🏗️ Build distribution code.`, treeObject, [parentCommit]).then(oid => {
  ...
  try {
    if (_nodegit.default.Branch.isCheckedOut(temporaryBranch)) throw new Error(`Cannot delete a checked out branch ${await temporaryBranch.name()}.`); // By reassigning the variable and looking up the branch the garbage collector will kick in. The reference for the branch in libgit2 shouldn't be in memory as mentioned in https://github.com/libgit2/libgit2/blob/859d92292e008a4d04d68fb6dc20a1dfa68e4874/include/git2/refs.h#L385-L398

    temporaryBranch = await _nodegit.default.Branch.lookup(repository, temporaryBranchName, 1); // referesh value of temporaryBranch - for some reason using the same reference prevents deletion of branch.
...
function rebasingExample({
  repository,
  branch,
  fromBranch,
  toBranch
}) {
  return repository.rebaseBranches(branch.name(), // branch commits to move
  fromBranch.name(), // till commits that are intersected with this branch (old branch)
  toBranch.name(), // onto the new branch.
  _nodegit.default.Signature.now('meow', 'test@example.com'), rebase => {
