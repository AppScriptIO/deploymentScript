...
module.exports = {
  ...
  inContainer2() {
    ...
    let processArg = [`run`, // `--volume /var/run/docker.sock:/var/run/docker.sock`,
    `--volume ${applicationHostPath}:/project/application`, `--volume ${applicationHostPath}:/project/scriptManager`, `--env hostPath=${applicationHostPath}`, `--name ${containerPrefix}`, `${image}`, `${containerCommand}`];
    ...
    let cp = (0, _child_process.spawn)(processCommand, processArg, {
      detached: false,
      shell: true,
      stdio: [0, 1, 2]
    });
