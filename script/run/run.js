...
var _sleepRun = _interopRequireDefault(require("./sleep.run.js"));
...
const serverSidePath = _configuration.default.directory.serverSidePath; // Install modules for the app to be run in the spawn app container 
...
switch (process.argv[0]) {
  ...
  default:
    ...
    /* disable loading built serverSide entrypoint */
    ? `${distributionServerSide}/entrypoint.js` : `${serverSidePath}/entrypoint.js`;
    console.log(`App enrypoint path: ${appEntrypointPath}`); // when using `localhost` chrome shows the files in folders, while using `0.0.0.0` files appear as separated.
    ...
    if (process.argv.includes('distribution')) Object.assign( // shallow merge
    environmentVariable, {
